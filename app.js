...
const TTSManager = {
  ...
  init() {
    ...
    // 사용 가능한 음성 목록에서 US/UK 영어 음성 찾기
    const findVoice = () => {
      const voices = window.speechSynthesis.getVoices();
      this.voice = voices.find(v => v.lang.startsWith('en') && (v.lang.includes('US') || v.lang.includes('GB') || v.lang.includes('UK'))) 
  ...
  speak(text, callback) {
    ...
    window.speechSynthesis.cancel(); // 진행중인 음성 취소

    const utterance = new SpeechSynthesisUtterance(text);
    if (this.voice) {
      utterance.voice = this.voice;
    }
    utterance.rate = 0.8; // 아이들을 위해 음성 속도를 0.8로 조금 낮춤
...
const Game = {
  ...
  loadQuestion() {
    ...
    let correctLetter = ALPHABET_LIST[Math.floor(Math.random() * ALPHABET_LIST.length)];
    ...
    if (this.currentLevel === 1) {
      ...
      // Level 3: [파닉스 - 기본] 알파벳과 파닉스 연결
      document.getElementById('choice-container').classList.remove('hidden');
      document.getElementById('question-instruction').textContent = "파닉스 소리를 듣고 알맞은 알파벳 카드를 고르세요.";
      
      this.currentQuestion = {
        correct: correctLetter,
        choices: choices,
        speechText: ALPHABET_DATA[correctLetter].phonicsTTS
      };
      ...
      // 무작위 단어 선택
      const wordObj = PHONICS_WORDS[Math.floor(Math.random() * PHONICS_WORDS.length)];
  ...
  handleAnswer(selected, buttonElement) {
    ...
    if (isCorrect) {
      ...
      else if (this.currentLevel === 3) levelName = 'Level 3 [파닉스 소리]';
      else if (this.currentLevel === 4) levelName = 'Level 4 [단어 완성]';
  ...
  setupLevel5() {
    ...
    // 5번 반복을 위한 타겟 선정
    const targetAlphabet = ALPHABET_LIST[Math.floor(Math.random() * ALPHABET_LIST.length)];
...
const CanvasDrawing = {
  ...
  drawGuideLetters(letter) {
    ...
    // 좌측 가이드 캔버스 (선명한 보라색 가이드)
    this.guideCtx.save();
    this.guideCtx.font = "bold 90px Fredoka, Outfit, sans-serif";
    this.guideCtx.textAlign = "center";
    this.guideCtx.textBaseline = "alphabetic";
    this.guideCtx.fillStyle = "#7c3aed"; // 진보라색
    
    // 대문자 그리기 (x=45)
    this.guideCtx.fillText(upper, 45, 180);
    // 소문자 그리기 (x=130)
    this.guideCtx.font = "bold 65px Fredoka, Outfit, sans-serif";
    this.guideCtx.fillText(lower, 130, 180);
    this.guideCtx.restore();
    
    // 우측 필기 캔버스 (연한 회색 따라 쓰기 가이드라인)
    this.ctx.save();
    this.ctx.font = "bold 110px Fredoka, Outfit, sans-serif";
    this.ctx.textAlign = "center";
    this.ctx.textBaseline = "alphabetic";
    this.ctx.fillStyle = "#cbd5e1"; // 아주 연한 회색 실루엣 (조금 선명하게 보정)
    
    // 대문자 따라쓰기 가이드 (x=120)
    this.ctx.fillText(upper, 120, 180);
    // 소문자 따라쓰기 가이드 (x=280)
    this.ctx.font = "bold 80px Fredoka, Outfit, sans-serif";
...
function downloadCSV(records) {
  const headers = ['학년', '반', '번호', '이름', '학습 일시', '레벨1', '레벨2', '레벨3', '레벨4', '레벨5', '오답 정보'];
...
async function buildTeacherTable() {
  ...
  records.forEach(row => {
    ...
    tr.innerHTML = `
      ...
      <td class="py-3 px-4 text-center font-bold ${row.level5Completed ? 'text-green-500' : 'text-slate-300'}">${row.level5Completed ? '🟢' : '⚪'}</td>
